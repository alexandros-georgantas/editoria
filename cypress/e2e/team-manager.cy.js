...
const addMember=(role) => {
    ...
    cy.get(".css-g1d714-ValueContainer").click().type(`${currentUser.username}{enter}`) 
    cy.get("[id='react-select-2-option-0']",{timeout:10000}).click({ force: true })
}
...
describe("Global Team manager settings", () => {
    ...
    it("Adding a Global Production Editor", () => {
        cy.get(".css-g1d714-ValueContainer").last().click().type(`${currentUser.username}{enter}`)
...
describe("Book specific Team manager settings", () => {
    ...
    it("Logging in with production editor privileges",()=>{
        ...
        // cy.get("[data-cy='Add_Component_Frontmatter']").should("exist");
        // cy.get("[data-cy='Add_Chapter_Body']").should("exist");
        // cy.get("[data-cy='Add_Part_Body']").should("exist");
        // cy.get("[data-cy='Add_Unnumbered_Body']").should("exist");
    ...
    it("Loging in with an account with author privilege",()=>{
        cy.login(currentUser.username,currentUser.password)
        cy.get("[title='User Menu dropdown']",{timeout:8000}).click()
        ...
        cy.contains("button","Archive").should("not.exist")
        cy.contains("a","Edit").click({force:true})
        cy.get('[title="Upload word files"]',{timeout:5000}).should("not.exist")
        cy.get('[title="Team Manager"]').should("not.exist")
        cy.get('[title="Metadata"]').should("exist")
        cy.get('[title="Asset Manager"]').should("exist")
        cy.get('[ title="Export Book"]').should("exist")  
        // cy.get('[title="Book Settings"]').should("exist");
        // cy.get("[data-cy='Add_Component_Frontmatter']").should("not.exist");
        // cy.get("[data-cy='Add_Chapter_Body']").should("not.exist");
        // cy.get("[data-cy='Add_Part_Body']").should("not.exist");
        // cy.get("[data-cy='Add_Unnumbered_Body']").should("not.exist");
