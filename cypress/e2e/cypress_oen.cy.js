...
describe('Testing OEN book builder', () => {
  ...
  it('Add a new book', () => {
    ...
    cy.get('h3')
      ...
      .siblings()
      .should('have.text', 'Before you start writing, prepare your book’s structure in four steps to create a consistent reading experience.① Determine Structure② Outline Content③ Add Pedagogical Elements④ Review TextbookCancelStart')
  ...
  it('Step 1-Determine Hierarchy: Chapters & Sections', () => {
    ...
    cy.get('h3')
      ...
      .next()
      .should('have.text', 'Textbooks are structured in hierarchical levels. Select the levels that will work best for your content. You can add more levels later when writing the book, if needed.')
    ...
    cy.contains("Are you sure you want to change the number of levels for the book with title OEN book?")
    cy.contains("If you change the number of levels then all the work which you might have done in following steps will be lost!")    
    cy.contains("Cancel").click()
  ...
  it('Step 2-Outline Content', () => {
    ...
    cy.get('h3')
      .should('have.text', '② Outline Content')
      .next()
      .should('have.text', 'Using the levels you defined in Step 1, write the outline for your textbook. You can add and name as many levels as you need to build the textbook’s content outline. Drag and drop the levels on the right to your textbook outline on the left.')

    cy.contains("Your Textbook Outline")
    
    // Adding title for each element
    cy.get("input[placeholder='Type the title of this part']").type("PART X")
    cy.get("input[placeholder='Type the title of this chapter']").type("CHAPTER X")
    cy.get("input[placeholder='Type the title of this section']").type("SECTION X")
    cy.get("input[placeholder='Type the title of this part']").should('have.value', "PART X")
    cy.get("input[placeholder='Type the title of this chapter']").should('have.value', "CHAPTER X")
    cy.get("input[placeholder='Type the title of this section']").should('have.value', "SECTION X")
    
    // Checking that Remove is disabled when there is only one element of a kind
    cy.get("input[placeholder='Type the title of this part']").parent().find('[title="Remove"]').should('be.disabled')
    cy.get("input[placeholder='Type the title of this chapter']").parent().find('[title="Remove"]').should('be.disabled')
    cy.get("input[placeholder='Type the title of this section']").parent().find('[title="Remove"]').should('be.disabled')
    
    // Adding structural parts

    // Cloning elements
    cy.get("input[placeholder='Type the title of this section']").parent().find('[title="Clone"]').click()
    cy.get("input[placeholder='Type the title of this chapter']").parent().find('[title="Clone"]').click()
    cy.get("input[placeholder='Type the title of this part']").parent().find('[title="Clone"]').click() 
...
*/
    ...
    // Adding elements
    cy.contains('Introduction').dragAndDrop(
      'span:first',
      'div:nth(19)'
    )
    cy.wait(4000) 

    cy.contains('Outline').dragAndDrop(
      'span:nth(1)',
      'div:nth(31)'
    )
    cy.wait(4000)
    ...
    cy.contains('Key Terms List').dragAndDrop(
      'span:nth(5)',
      'div:nth(19)'
    )
    cy.wait(4000) 
  
    cy.contains('Summary').dragAndDrop(
      'span:nth(8)',
      'div:nth(31)'
    )
    cy.wait(4000) 
  ...
  it('Step 4-Review textbook', () => {
    ...
    cy.get('h3')
      ...
      .next()
      .should('have.text', 'This is the complete outline and structure of your textbook. Review and edit as needed. If you’d like to change the pedagogical elements you’ve selected, return to Step 3.')
    ...
    cy.get("input[placeholder='Type the title of this part']:first").click().clear().type("First Part")
    cy.get("input[placeholder='Type the title of this part']:nth(1)").click().clear().type("Second Part")
    ...
    cy.contains('You will no longer be able to access the Open Textbook Planner, but you can make any changes you’d like in the book after it is built.')
  ...
  it('Check Editor', () => {
    ...
    cy.get('div[data-type="content_structure_element"]').should('have.class', 'introduction')
    cy.get('button[title="Outline"]').click({force:true})
    cy.get('div[data-type="content_structure_element"]').should('have.class', 'outline')
    cy.get('button[title="Learning Objectives"]').click({force:true})
    cy.get('div[data-type="content_structure_element"]').should('have.class', 'learning-objectives')
    cy.get('button[title="Focus Questions"]').click({force:true})
    cy.get('div[data-type="content_structure_element"]').should('have.class', 'focus-questions')
    cy.get('button[title="Content Opener Image"]').click({force:true})
    cy.get('div[data-type="content_structure_element"]').should('have.class', 'content-opener-image')

    // Checking Openers and Closers
    cy.get('button[title="Key Terms List"]').click({force:true})
    cy.get('div[data-type="content_structure_element"]').should('have.class', 'key-terms')
    cy.get('button[title="Self-reflection Activities"]').click({force:true})
    cy.get('div[data-type="content_structure_element"]').should('have.class', 'self-reflection-activities')

    // Checking Closers
    cy.get('button[title="Review Activity"]').click({force:true})
    cy.get('div[data-type="content_structure_element"]').should('have.class', 'review-activity')
    cy.get('button[title="Summary"]').click({force:true})
    cy.get('div[data-type="content_structure_element"]').should('have.class', 'summary')
    cy.get('button[title="References"]').click({force:true})
    cy.get('div[data-type="content_structure_element"]').should('have.class', 'references')
    cy.get('button[title="Bibliography"]').click({force:true})
    cy.get('div[data-type="content_structure_element"]').should('have.class', 'bibliography')
    cy.get('button[title="Further Reading"]').click({force:true})
    cy.get('div[data-type="content_structure_element"]').should('have.class', 'further-reading') 
    ...
    // Checking Long Boxes
    cy.get('button[title="long"]').contains('Case Study').click({force:true})
    cy.get('aside').should('have.class', 'long case-study')
    cy.get('button[title="long"]').contains('Biography').click({force:true})
    cy.get('aside').should('have.class', 'long biography')
    cy.get('button[title="long"]').contains('Worked Example').click({force:true})
