...
/*        cy.get(".css-g1d714-ValueContainer").then(els => {
        ...
        cy.get(`[id='thumbnail-upload-btn']`).selectFile(data.thumbnail, {force: true})
        cy.get(`[id="files-upload-btn"]`).selectFile(data.file_path, {force: true})
    ...
    //    cy.get('.css-g1d714-ValueContainer:nth(1)').should("have.text", "Script2")
        ...
        cy.get('button[title="Delete file"]').siblings().should('have.text', data.file_name)
    })
    ...
    it("Editing template content, files and thumbnail",()=>{
        ...
        cy.get(`[id='thumbnail-upload-btn']`).selectFile(data.thumbnail2, {force: true})

        // Adding and removing template file
        cy.log("Removing template files")
        cy.get(`[id="files-upload-btn"]`).selectFile(data.temp_file, {force: true});
    ...
    it("Checking if the updated content is retained",()=>{
        ...
        cy.get('button[title="Delete file"]').siblings().should('have.text', data.temp_file_name)
    })

    it("Deleting template",()=>{
        cy.get('button[title="Delete"]').first().click({force:true})
        cy.contains(`Are you sure you want to delete the template with name ${data.name2}?`)
