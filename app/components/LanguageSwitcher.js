...
const LanguageSwitcher = () => {
  ...
  const { t, i18n } = useTranslation()
    useEffect(() => {
        // Calculate if the dropdown should be positioned above or below the button
        const dropdownRect = dropdownRef.current.getBoundingClientRect();
        setIsBelow(dropdownRect.bottom + 10 < window.innerHeight);
    }, []);

  return (
    <LanguageSwitcherWrapper>
      <GlobeIcon />
      <DropdownMenu isBelow={isBelow} ref={dropdownRef}>
        <DropdownMenuItem>
