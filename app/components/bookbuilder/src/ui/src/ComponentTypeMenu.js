...
const ComponentTypeMenu = ({
    ...
    if (clonedDivision.name === 'Body') {
      if (bookStructure.levels.length === 3) {
        clonedDivision.allowedComponentTypes.splice(0, 0, {
          predefined: true,
          title: bookStructure.levels[0].displayName,
          value: bookStructure.levels[0].type,
      ...
      if (bookStructure.levels.length === 4) {
        ...
        clonedDivision.allowedComponentTypes.splice(1, 0, {
          predefined: true,
          title: bookStructure.levels[1].displayName,
          value: bookStructure.levels[1].type,
          visibleInHeader: true,
        })
      }
    }
  ...
  const groupedOptions = groupBy(clonedDivision.allowedComponentTypes, value =>
    value.predefined ? 'predefined' : 'custom',
  )

  // delete endnotes from menu options
  if (divisionType === 'Backmatter') {
