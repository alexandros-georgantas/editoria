...
const DeleteBookComponentModal = props => {
  ...
  return (
    <DialogModal
      buttonLabel="yes"
      headerText={`${t('Delete')} ${t(componentType)}`}
      isOpen={isOpen}
      onConfirm={onConfirm}
    ...
    >
      ...
      */}
      <Text>
        {t(
          'are_you_sure_you_want_to_delete_this {{component_type}} with_title {{title}}?',
          confirmParameters,
        )}
        <br />
        {componentType === 'endnotes' &&
          t(
            'by_doing_so_you_will_not_be_able_to_see_templates_with_notes_option_endnotes',
          )}
      </Text>
    </DialogModal>
  )
